...
export default function StaffCommissions() {
  ...
  const clearRow = async (commissionId: string) => {
    if (IS_PREVIEW) {
      alert('Demo mode — in production this would mark paid and notify the agent.');
      return;
    }
  ...
  return (
    <div className="container py-6 sm:py-8 space-y-6 sm:space-y-8">
      <div>
        ...
        <h1 className="font-heading text-3xl mt-1">Commissions to clear</h1>
        <p className="text-ink-soft mt-1">Your agents&apos; commissions — mark as paid once the money&apos;s out.</p>
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-3">
        ...
        <KPI icon={<TrendingUp className="h-5 w-5" />} label="Total" value={formatNGN(totals.total)} tint="green" />
      </div>
